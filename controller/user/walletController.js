...
const wallet = async (req, res) => {
  ...
  try {
    ...
    if (user && user.length > 0) {
      ...
      res.render("users/wallet", { user: req.session.userExist, wallet: null });
    }
  } catch (error) {
    console.error(error);
  }
    ...
    if (isPaymentValid) {
      ...
      if (!payment || !payment.amount) {
        return res
          ...
          .json({
            success: false,
            message: "Invalid payment or missing amount.",
          });
      }
...
const claimRefferal = async (req, res) => {
  try {
    ...
    if (!userWallet) {
      ...
      claimingUser.walletId = userWallet._id;
    } else {
    ...
    if (!referrerWallet) {
      ...
      referrerUser.walletId = referrerWallet._id;
    ...
    res
      ...
      .json({
        success: false,
        message: "An error occurred while claiming the referral.",
      });
  }
};
